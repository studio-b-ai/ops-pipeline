...
| 15b | **acuops-pipeline#124** — Rule #34 snapshot-integrity check LOST in the split: the LIVE engine's snapshot.py (client-asthetik pins @v1) saves backup zips unverified, "Snapshot saved: ok" unconditional + exit-0-on-failure — the exact April corrupt-snapshot lie, on today's Heritage deploy path. Small fix + v1-tag ride (#37) | CTO next sitting |
