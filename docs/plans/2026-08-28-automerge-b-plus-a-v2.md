...
5. **Independent review:** ~~the existing sha-pinned Sonnet review (`independentReview` over `fetchDiffBySha(headRefOid)`) returns exactly CLEAN.~~ **SUPERSEDED 2026-09-19 (stint #372):** the 2026-09-15 "Box is the one key" ruling (law 1) makes `box` open EVERY decision leg — review included; the floor (step 4's CI + mergeStateStatus CLEAN) never lowers. The train gate no longer runs a model vote: a FLAG could only ever refuse against the authority the gate had already verified (the live gap: client-asthetik#391 refused 07:0xZ on "review verdict FLAG 0/3" *with Kevin's `box` present*), and a re-vote under a flag was the #724 re-roll failure mode. The squasher path's `evaluateQueuedOverride` has honored box this way since ops-pipeline#260 leg 4.
...
- **Trigger:** `pull_request` workflow event, `types: [closed]`, `merged == true`, PR carries the `automerge:code-fix` label — the label is only a cheap **filter** deciding which merges to evaluate, and carries zero authority. **Trigger verdict = re-derived predicate, not identity (codex v2 P2-4 + pass-2):** identity provenance is vacuous under the shared `kbibelhausen` login (gate, squasher, and Kevin are indistinguishable within one login), so the tripwire instead RE-RUNS the code-fix classification against server state — sha-pinned diff of the merged PR, author `BUGSQUASHER_AUTHOR`, `bugsquasher` label, ≤150 lines, `safe_path_globs`, denylist, named checks — exactly the §4.1 predicate. Verdict ≠ code-fix ⇒ no tripwire run, receipt "not code-fix class". A forged label on an arbitrary PR fails classification (false-revert noise dead); a hand-merged PR that genuinely satisfies the predicate gets a tripwire watch — protective, not harmful, and its revert PR still requires a human (#97). This is the design law applied: the decision re-derives from server state; no label or login is trusted for anything but filtering. Squash sha = `event.pull_request.merge_commit_sha` (move 3; D3 killed).
...
| B2 tripwire | Merge a plant that deploys clean → tripwire PASS receipt bound to the correct deployment id | forced 5xx burst on a throwaway service → revert PR OPENS (and is NOT auto-merged); hand-applied `automerge:code-fix` on a merged PR that fails the re-derived predicate → "not code-fix class" refusal receipt |
