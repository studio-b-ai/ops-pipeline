...
5. **Independent review:** ~~the existing sha-pinned Sonnet review (`independentReview` over `fetchDiffBySha(headRefOid)`) returns exactly CLEAN.~~ **SUPERSEDED 2026-09-15 (box ruling):** `box` opens EVERY decision leg, review included — the train gate no longer runs the model vote; only the CI rollup / mergeable floor (step 4) and the revalidate (step 7) stand. This is the squasher path's own law (`evaluateQueuedOverride` has skipped the review leg since ops#260; toto#24 merged that way) applied to the train gate, closing the door gap that refused client-asthetik#391 on `review verdict FLAG (0/3 CLEAN)` with Kevin's `box` present (run 34939031962). The B-side squasher gate's review leg is unchanged.
