...
def pr_cards():
    ...
    for p in prs:
        ...
        if still_refused(L):
            ...
            # stint #410: the door's own "Box it" template is never a card. A still-refused PR
            # only reaches Kevin's queue when the RE has written a real card file.
            bay = "studio-b" if p["repo"] in ("claude-config-plane","ops-pipeline","brain","power-unit","toto","lightsout","radio") else "asthetik"
            card_fn = f"pr-{p['repo']}-{p['n']}.md"
            card_path = os.path.join(BRAIN, "receipts", "cards", bay, card_fn)
            p["flag"] = "refused"
            if os.path.exists(card_path):
                ...
                flags.append(p)   # no real read yet — the door's own text is never a card → RE's row
            continue
