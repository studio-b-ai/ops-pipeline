...
import { gateDecision, type GateInput } from "../automerge-classify.js";
...
const base: Omit<GateInput, "labels"> = {
  files: [{ path: "docs/x.md", fileClass: "doc" }],
  totalChangedLines: 3,
  author: "kbibelhausen",
  ciClean: true,
  reviewVerdict: "CLEAN",
};
const labelReasons = (labels: string[]) => gateDecision({ ...base, labels }).reasons.filter((r: string) => r.includes("eligibility label"));
...
describe("eligibility label — both directions", () => {
  ...
  it("known-bad: neither label is refused at the label leg", () => {
    ...
    expect(r.reasons.some((x: string) => x.startsWith("missing an eligibility label"))).toBe(true);
