...
const ALLOWLIST = new Set(["studio-b-ai/bolt-wms", "studio-b-ai/studiob", "studio-b-ai/studiob-price-sync", "studio-b-ai/webhook-router", "studio-b-ai/asthetik-website"]);
...
describe("laneLabelFor (2026-09-06 — every route carries the seat's lane label)", () => {
  it("known-GOOD: the five router-allowlisted repos all resolve to lane:engineer (the seat that reads those queues)", () => {
    for (const r of ["bolt-wms", "studiob", "studiob-price-sync", "webhook-router", "asthetik-website"]) {
